...
sub handler { 
  ...
  # Embian Patch
	...
	# FORCEMAX ADD 20100523
	#$buf = $content;
	$buf = Encode::decode("utf-8", $content);
#	print STDERR "$buf \n";
	#   
 
	my $new_len = length($content);
	#print STDERR "Content Length : $new_len \n";
...
BEGIN {
	...
	sub as_string {
               ...
               $value =~ s/</&lt;/g;
               $value =~ s/>/&gt;/g;
#	      print STDERR "[JSLEE] as_string value: $value \n";
